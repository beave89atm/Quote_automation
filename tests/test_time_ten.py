...
Kyle confirmed all 10 Time LOM Excels. Those sheets are takeoff truth.
Time method is locked: clip LIST OF MATERIAL, cell-by-cell.

102728-1 qty (51/97) is still first — live row-qty sum must be 97, not 30.
The other nine are Kyle-locked at their dash-1 / single-column bars.
105098-1 is parent A–J only (9/9); do not ingest later-sheet 103603-1
child tables. Remaining PNs on partial fixtures were not listed — do
not invent them. Live proof is the laptop clip → xlsx. Do not merge
until live extract_bom matches all 10.
"""
...
from tests.test_bom_table import (
    ...
    _KYLE_1007922_1,
    _KYLE_1007922_1_PCS,
    _KYLE_1007922_1_PN_COUNT,
    _KYLE_33612_1,
    _KYLE_33612_1_PCS,
    _KYLE_33612_1_PN_COUNT,
    _KYLE_105098_1,
    _KYLE_105098_1_PCS,
    _KYLE_105098_1_PN_COUNT,
    _KYLE_P904225_1,
    _KYLE_P904225_1_PCS,
...
def _specs() -> dict[str, TimeTenSpec]:
    return {
        ...
        "1004611": TimeTenSpec(
            ...
            extra_cells=_kyle_1004611_cell_rows()[1:],
            status="kyle_locked",
        ),
        "103516": TimeTenSpec(
            key="103516",
            ...
            extra_cells=_kyle_103516_cell_rows()[1:],
            status="kyle_locked",
        ),
        "105098-1": TimeTenSpec(
            ...
            extra_cells=_kyle_105098_cell_rows()[1:],
            status="kyle_locked",
        ),
        "33612-1": TimeTenSpec(
            key="33612-1",
            ...
            kyle_locked=True,
            header=["QTY", "ITEM", "PART NO.", "DESCRIPTION"],
            rows=list(_KYLE_33612_1),
            reject_pns=frozenset({"33612-1", "33612", "56657", "97879", "72143"}),
            extra_cells=_kyle_33612_cell_rows()[1:],
            status="kyle_locked",
        ),
        "21727-1": TimeTenSpec(
            key="21727-1",
        ...
        "1007922-1": TimeTenSpec(
            ...
            extra_cells=_kyle_1007922_cell_rows()[1:],
        ...
        "P904225-1": TimeTenSpec(
            ...
            bom_config="",
            kyle_locked=True,
            header=["P904225-1", "ITEM", "PART NO.", "DESCRIPTION"],
            rows=list(_KYLE_P904225_1),
            reject_pns=frozenset(
                {
                    "P904225-1",
                    "904225-1",
            ...
            extra_cells=_kyle_p904225_cell_rows()[1:],
...
def _cells_for(spec: TimeTenSpec) -> list[list[str]]:
    ...
    if spec.key == "1004611":
        return _kyle_1004611_cell_rows()
    ...
    if spec.key == "1007922-1":
        return _kyle_1007922_cell_rows()
    if spec.key == "33612-1":
        return _kyle_33612_cell_rows()
    if spec.key == "105098-1":
        return _kyle_105098_cell_rows()
...
def test_time_ten_order_is_kyle_order():
    ...
    assert specs["1007922-1"].kyle_locked
    assert specs["33612-1"].kyle_locked
    assert specs["105098-1"].kyle_locked
    assert all(specs[k].kyle_locked for k in TIME_TEN_ORDER)
    ...
    assert {pn for _i, _q, pn, _d in _KYLE_1004611_1} == {"1004611-DWG", "80054-1"}
    assert "1004620-2" not in {pn for _i, _q, pn, _d in _KYLE_1004611_1}
    assert _KYLE_P904225_1_PN_COUNT == 11
    assert _KYLE_P904225_1_PCS == 23
    assert {pn for _i, _q, pn, _d in _KYLE_P904225_1} == {"89100-1", "P904226-1"}
    assert "89176-1" not in {pn for _i, _q, pn, _d in _KYLE_P904225_1}
    assert "P904225-1" not in {pn for _i, _q, pn, _d in _KYLE_P904225_1}
    ...
    assert {pn for _i, _q, pn, _d in _KYLE_1007922_1}.isdisjoint(
        {"21750-2", "21743-2", "73207"}
    )
    assert _KYLE_33612_1_PN_COUNT == 21
    assert _KYLE_33612_1_PCS == 47
    assert {pn for _i, _q, pn, _d in _KYLE_33612_1} >= {
        "89176-1",
        "94560",
        "28275-1",
        "28275-2",
    }
    assert "56657" not in {pn for _i, _q, pn, _d in _KYLE_33612_1}
    assert "97879" not in {pn for _i, _q, pn, _d in _KYLE_33612_1}
...
def test_103516_hyphenless_dupes_and_column_index_are_not_pieces():
    spec = _specs()["103516"]
    cells = _cells_for(spec)
    cells.append(["", "1", "96", "103537-1", "TUBE"])
    cells.append(["", "1", "99", "1035371", "TUBE"])
    cells.append(["", "1", "98", "1035221", "PLATE"])
    cells.append(["", "1", "97", "1035281", "RAIL"])
    bom = parse_material_list_cells(cells, bom_config="-1")
    parts = {r.part_no for r in bom.rows}
    assert "103535-1" in parts
    assert "40002-2" in parts
    assert "103537-1" in parts
...
def test_time_ten_is_not_live_done():
    ...
    assert specs["1004611"].bom_config == "-1"
    ...
    assert "11694-2" not in parts
    assert "25009-2" not in parts
    assert specs["1004611"].rows[0] == ("A", 1, "1004611-DWG", "")
    assert specs["1004611"].rows[1][2] == "80054-1"
    assert _KYLE_1004611_1_PN_COUNT == 22
    assert _KYLE_1004611_1_PCS == 66
    assert specs["P904225-1"].bom_config == ""
    assert specs["P904225-1"].header[0] == "P904225-1"
    assert specs["P904225-1"].rows[0][2] == "89100-1"
    assert "89176-1" not in {pn for _i, _q, pn, _d in specs["P904225-1"].rows}
    assert _KYLE_P904225_1_PN_COUNT == 11
    assert _KYLE_P904225_1_PCS == 23
    assert specs["103516"].bom_config == "-1"
    assert specs["103516"].header[:3] == ["-2", "-1", "ITEM"]
    assert specs["103516"].rows[-1] == ("27", 1, "40002-2", "")
    assert specs["103516"].rows[0][2] == "103535-1"
    ...
    assert _KYLE_21727_1_PN_COUNT == 11
    assert _KYLE_21727_1_PCS == 16
    assert specs["1007922-1"].bom_config == "-1"
    assert specs["1007922-1"].header[:3] == ["-2", "-1", "ITEM"]
    assert specs["1007922-1"].rows[1] == ("B", 4, "14149-1", "FILLER")
    assert specs["1007922-1"].rows[2] == ("C", 2, "1007830-1", "OUTRIGGER LEG")
    assert {pn for _i, _q, pn, _d in specs["1007922-1"].rows}.isdisjoint(
        {"21750-2", "21743-2", "73207"}
    )
    assert _KYLE_1007922_1_PN_COUNT == 6
    assert _KYLE_1007922_1_PCS == 14
    assert specs["33612-1"].bom_config == ""
    assert specs["33612-1"].header == ["QTY", "ITEM", "PART NO.", "DESCRIPTION"]
    assert specs["33612-1"].rows[0] == ("A", 1, "89176-1", "TUBE")
    assert specs["33612-1"].rows[1] == ("M", 1, "94560", "GATE, FABRICATION")
    assert {pn for _i, _q, pn, _d in specs["33612-1"].rows} >= {
        "28275-1",
        "28275-2",
        "28275-3",
    }
    assert "56657" not in {pn for _i, _q, pn, _d in specs["33612-1"].rows}
    assert "97879" not in {pn for _i, _q, pn, _d in specs["33612-1"].rows}
    assert _KYLE_33612_1_PN_COUNT == 21
    assert _KYLE_33612_1_PCS == 47
    assert specs["105098-1"].bom_config == ""
    assert specs["105098-1"].header == ["QTY", "ITEM", "PART NO.", "DESCRIPTION"]
    assert specs["105098-1"].rows == []
    assert "103603-1" in specs["105098-1"].reject_pns
    assert _KYLE_105098_1_PN_COUNT == 9
    assert _KYLE_105098_1_PCS == 9
