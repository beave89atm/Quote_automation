...
def test_preserve_operation_cost_lists_merges_by_id():
    client = MagicMock()
    # Outbound payload lost Profile (stale).
    outbound = {
        "ItemList": [
            {
                "ID": "p1",
    ...
    # Fresh server still has Profile.
...
def test_pdf_assembly_does_not_relink_after_profile():
    ...
    # Find build_pdf_only_assembly body after the Profile attach.
    fn = src.split("def build_pdf_only_assembly", 1)[1]
    # Truncate at next top-level def.
    body = fn.split("\ndef ", 1)[0]
