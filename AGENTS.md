...
- [`.cursor/environment.json`](.cursor/environment.json) installs Python + frontend, then starts uvicorn on `0.0.0.0:8000` (Desktop: `http://127.0.0.1:8000`).
- `.env` is gitignored. On boot, `start` runs [`scripts/cursor_cloud_write_env.sh`](scripts/cursor_cloud_write_env.sh), which writes `.env` only when Cursor secrets `SECTURAFAB_CLIENT_ID` and `SECTURAFAB_CLIENT_SECRET` are set. You can also drop a `.env` onto the VM; it is never committed.
